-- Keep a single unambiguous admin product editor and separate archive restoration.

drop function if exists public.admin_update_product(uuid, text, text, integer, public.product_status, integer);
drop function if exists public.admin_update_product(uuid, text, text, integer, public.product_status, integer, integer, integer);
drop function if exists public.admin_update_product(uuid, text, text, text, integer, public.product_status, integer, integer, integer);
drop function if exists public.admin_update_product(uuid, text, text, text, integer, public.product_status, integer, uuid, integer, integer);
...
create or replace function public.admin_restore_product(_product_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  ...
  update public.products
  set status = case when available_stock > 0 then 'active'::public.product_status
                    else 'out_of_stock'::public.product_status end
  where id = _product_id and status = 'discontinued';
  if not found then raise exception 'Producto descontinuado no encontrado'; end if;

  insert into public.admin_activity_log (admin_id, action, entity_type, entity_id)
  values (auth.uid(), 'product_restored', 'product', _product_id);
end;
$$;

revoke all on function public.admin_restore_product(uuid) from public, anon;
grant execute on function public.admin_restore_product(uuid) to authenticated;
